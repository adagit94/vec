...
export const density: Density = ({ values, xInterval, yInterval, width, height }) => {
    ...
    let maxXVal = 0, maxYVal = 0

    for (const [xVal, yVal] of values) {
        maxXVal = Math.max(xVal, maxXVal)
        maxYVal = Math.max(yVal, maxYVal)
    }
